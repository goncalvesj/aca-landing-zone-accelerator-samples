...
@description('Optional. The name of the subnet to create for the spoke db. If set, it overrides the name generated by the template.')
param spokePostgresSubnetName string = 'snet-postgres'

@description('CIDR of the spoke db subnet.')
param spokePostgresSubnetAddressPrefix string
...
var defaultSubnets = [
  ...
  {
    name: spokePostgresSubnetName
    properties: {
      addressPrefix: spokePostgresSubnetAddressPrefix
      networkSecurityGroup: {
        id: nsgPostgres.outputs.nsgId
      }
      delegations: [
        {
          name: 'dbdelegation'
...
@description('NSG Rules for the db subnet.')
module nsgPostgres '../../../../shared/bicep/network/nsg.bicep' = {
  name: take('nsgPostgres-${deployment().name}', 64)
  scope: spokeResourceGroup
  params: {
    name: naming.outputs.resourcesNames.postgresNsg
    location: location
    tags: tags
    securityRules: []
    diagnosticWorkspaceId: logAnalyticsWorkspace.outputs.logAnalyticsWsId
...
resource vnetSpokeCreated 'Microsoft.Network/virtualNetworks@2022-07-01' existing = {
  ...
  resource spokePostgresSubnet 'subnets' existing = {
    name: spokePostgresSubnetName
  }
...
@description('The resource ID of the spoke db subnet.')
output spokePostgresSubnetId string = vnetSpokeCreated::spokePostgresSubnet.id
